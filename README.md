...
> new in v0.6 — the design system applied consistently across every page (admin
> included) plus an accessibility pass (skip link, a text-size/high-contrast
> setting, a tablet breakpoint). See
...
command (v0.5), and — new in v0.6 — the design system applied across every page
plus an accessibility pass. The path from here stays lean and open (no accounts,
no profiles, no tracking) — see **[ROADMAP.md](ROADMAP.md)** for what's next and
