---
id: crafts-cordage
title: Make rope and cordage from plant fibre
category: crafts
summary: Turn plant fibres into strong cord by reverse-wrap twisting.
---

# Make rope and cordage from plant fibre

Cord and rope are among the most useful things you can make: they tie shelters,
haul loads, make snares and fishing lines, repair tools, and bind almost
anything. With the reverse-wrap method you can turn plant fibre into surprisingly
strong cordage by hand.

> **Strength comes from the twist.** Loose fibres are weak; twisted and
> counter-twisted, the same fibres grip each other and become rope.

## Gather and prepare fibre

Good fibre sources include the inner bark of some trees, nettle stems, dogbane,
yucca and agave leaves, and many long, tough grasses and plant stalks.

1. **Collect** long, fibrous material.
2. **Separate the fibres:** strip the usable long fibres from the woody or fleshy
   parts. Drying then lightly crushing and rolling the stems helps.
3. **Soak or dampen** stiff fibres slightly so they bend without snapping.

## Reverse-wrap twisting

This is the core technique:

1. Take a bundle of fibres and twist it tight until it kinks and folds back on
   itself, forming two strands meeting at a loop.
2. Hold the loop. **Twist the top strand away from you**, then **fold it over
   toward you** across the other strand.
3. Repeat with what is now the top strand: twist away, fold over toward you.
4. Keep alternating. The opposing twists lock together into cord.

## Make it longer and thicker

- **Splice in new fibre** before a strand runs out: lay new fibres alongside the
  thinning strand and twist them in. Stagger the joins so they never line up.
- **For thicker rope,** make several lengths of cord, then reverse-wrap *those*
  together the same way.

## Finish and care

- Tie off or tuck the ends so they can't unravel.
- Let cordage dry fully, then store it dry and coiled.
- Test any cord before trusting it with weight or with people.
