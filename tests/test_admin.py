...
def test_admin_enabled_by_default_with_autogenerated_token(monkeypatch, tmp_path):
    """With no configured token, horizon generates and persists one on first use."""
    ...
    monkeypatch.setattr("horizon.config.settings.data_dir", str(tmp_path), raising=False)
    monkeypatch.setattr(admin_module, "_cached_autogenerated_token", None)

    with TestClient(app) as client:
        # Login page offers the form (admin is not disabled).
        page = client.get("/admin/login")
        assert page.status_code == 200
        assert "sign in" in page.text.lower()

        generated_token = (tmp_path / "admin_token").read_text(encoding="utf-8").strip()
        assert generated_token

        resp = client.post("/admin/login", data={"token": generated_token})
        assert resp.status_code == 200
        assert "Content dashboard" in resp.text

        # The same token is reused across the process (persisted to disk).
        assert admin_module._effective_token() == generated_token
